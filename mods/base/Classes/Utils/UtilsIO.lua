...
function io.remove_directory_and_files(path, verbose)
	if not path then
		BLT:Log(LogLevel.ERROR, "UtilsIO", "Paramater #1 to io.remove_directory_and_files, string expected, received",
			path)
		return false
	end
...
function FileIO:CopyToAsync(path, to_path, callback)
	self:CopyFilesToAsync(self:PrepareFilesForCopy(path, to_path), callback or function(success, message)
		if success then
			log(string.format("[FileIO] Done copying directory %s to %s", path, to_path))
		else
			log(string.format("[FileIO] Something went wrong when copying directory %s to %s, \n %s", path, to_path,
				message))
		end
	end)
end
