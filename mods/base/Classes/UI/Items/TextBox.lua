BLT.Items.TextBox = BLT.Items.TextBox or class(BLT.Items.Item)
local TextBox = BLT.Items.TextBox
...
function TextBox:SetValueByMouseXPos(x)
	...
	if self.menu._old_x ~= x then
		...
		elseif managers.mouse_pointer._mouse:world_x() == 0 then
			managers.mouse_pointer:set_mouse_world_position(self.menu._panel:w() - 1,
				managers.mouse_pointer._mouse:world_y())
			self.ignore_next = true
		else
