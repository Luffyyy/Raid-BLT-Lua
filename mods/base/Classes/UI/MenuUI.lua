...
function MenuUI:init(params)
    if not managers or not managers.gui_data then
    ...
    BLT:AddUpdater("MenuUIUpdate" .. tostring(self), callback(self, self, "Update"), true)
...
function MenuUI:Group(params)
    return self:AddMenu(BLT.Items.Group:new(self:ConfigureMenu(params)))
end

function MenuUI:DivGroup(params)
    local _params = self:ConfigureMenu(params)
    _params.divider_type = true
    return self:AddMenu(BLT.Items.Group:new(_params))
end

function MenuUI:Menu(params)
    return self:AddMenu(BLT.Items.Menu:new(self:ConfigureMenu(params)))
end
